...
import { mergeMultipleTexts } from './services/mergeService';
...
function App() {
  ...
  const updateInput = (id: string, value: string) => {
    ...
    // Adjust textarea height after value update
    requestAnimationFrame(() => adjustTextareaHeight(id));
  };
  ...
  // Ensure textareas adjust height on mount / when inputs array changes
  useEffect(() => {
    // small timeout to wait for DOM nodes to be attached
    const raf = requestAnimationFrame(() => {
      inputs.slice(0, 2).forEach(i => adjustTextareaHeight(i.id));
    });
  ...
  // View: Result Editor
  if (segments) {
    // We only support two versions now; always label them Version 1 and Version 2
    const labelA = "Version 1";
    const labelB = "Version 2";
      ...
      <header className="bg-white border-b border-tech-light/30 px-6 py-4 flex flex-col md:flex-row items-center justify-between sticky top-0 z-30 backdrop-blur-sm bg-white/90 shadow-sm gap-4 md:gap-0">
        <div className="flex items-center gap-3 w-full md:w-auto">
          ...
          <div>
            ...
            <p className="text-xs text-tech-dim">Gabungkan Dua Versi Teks</p>
          </div>
        </div>
          ...
          <div className="max-w-6xl mx-auto">
            ...
            <div className="flex flex-col md:flex-row gap-6">
              {inputs.slice(0, 2).map((input, index) => (
                <div key={input.id} className="flex-1 flex flex-col bg-white rounded-xl shadow-sm border border-tech-light hover:border-tech-dim transition-all">
                  <div className="p-3 border-b border-tech-light/30 flex justify-between items-center bg-gray-50/50 rounded-t-xl">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-xs text-tech-light font-mono">{input.value.length} karakter</span>
                    </div>
                  </div>
                  <textarea
                    ref={(el) => (textareaRefs.current[input.id] = el)}
                    className="w-full p-4 resize-none bg-white focus:outline-none focus:bg-gray-50/30 transition-colors font-mono text-sm leading-relaxed text-tech-black placeholder:text-tech-light/60 rounded-b-xl border-none focus:ring-0 overflow-hidden"
                    placeholder={`Tempel teks untuk versi ${index + 1} di sini...`}
                    value={input.value}
                    onChange={(e) => updateInput(input.id, e.target.value)}
                    onInput={() => adjustTextareaHeight(input.id)}
                    style={{ height: 'auto' }}
                  />
                </div>
              ))}
          ...
          <div className="max-w-4xl mx-auto flex flex-col items-center gap-4">
            ...
            <div className="flex flex-col items-center w-full gap-4">
              ...
              <div className="flex gap-4 w-full md:w-auto">
                ...
                >
                  {isProcessing ? 'Menggabungkan...' : `Gabungkan 2 Versi`}
                </Button>
              </div>
            </div>

            <p className="text-xs text-tech-light text-center max-w-lg font-sans">
              Gabungkan dua versi teks. Konflik akan ditandai antara kedua versi.
            </p>
          </div>
        </div>
      </main>
